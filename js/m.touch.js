...
function MScroll(init){
	...
	this.minTranslate = {x: this.element.clientWidth - this.Scroll.offsetWidth, y: this.element.clientHeight - this.Scroll.offsetHeight};
	this.scale = {x:this.element.clientWidth  / this.Scroll.offsetWidth,y:this.element.clientHeight / this.Scroll.offsetHeight};
	this.scrollBar = {};
	this.isDir = {x:true,y:true};
	if(this.showBar) {
		if(this.dir == "x"){
			setBar("x");
		} else if(this.dir == "y") {
	...
	function setBar(dir){
		_this.scrollBar[dir] = document.createElement("div");
		_this.scrollBar[dir].id = "scrollBar"+(dir.toUpperCase());
		_this.scrollBar[dir].style.cssText="position:absolute;background:rgba(0,0,0,.5);border-radius:2px;opacity:0; transition:.2s opacity;";
		if(dir == "x"){
			var width =   _this.element.clientWidth * _this.scale.x +"px"
			_this.scrollBar[dir].style.cssText+="height:4px;min-width:4px;left:0;bottom:0; width:"+width;
		} else {
			var height =  _this.element.clientHeight * _this.scale.y +"px"
			_this.scrollBar[dir].style.cssText+="width:4px;right:0;top:0;min-height:4px;height:"+height;
		}
		_this.element.appendChild(_this.scrollBar[dir]);
	}
...
MScroll.prototype = {
	toStart: function(e){ 
		...
		this.Scroll.style.transition = "0ms";
		this.scrollBar.y&&(this.scrollBar.y.style.transition = ".2s opacity");
		this.scrollBar.x&&(this.scrollBar.x.style.transition = ".2s opacity");
		var touch = e.changedTouches[0];
		this.startPage = {y:touch.pageY ,x:touch.pageX};
		this.startTranslate.x = this.iScroll.x;
		this.startTranslate.y = this.iScroll.y;
		this.lastTime = new Date().getTime(); // 100
		this.lastTranslate.x = this.iScroll.x;
		this.lastTranslate.y = this.iScroll.y;
		this.timeDis = 0;
		if(this.dir == "x"){
			this.isDir.x = true;
			this.isDir.y = false;
		...
		this.translateDis = {x:0,y:0}; 
		if(this.scrollBar.x&&this.showBar){
			this.scrollBar.x.style.opacity = 1;
		}
		if(this.scrollBar.y&&this.showBar){
			this.scrollBar.y.style.opacity = 1;
		}
	},
	toMove: function(e){  /* 用户手指移动，跟随用户手指滑动 */
		...
		var _this = this;
		var touch = e.changedTouches[0];
		var nowPage = {y:touch.pageY, x:touch.pageX};
		var nowTime = new Date().getTime();
		if(Math.abs(nowPage.y - this.startPage.y) - Math.abs(nowPage.x - this.startPage.x) > 0) {
			this.isDir.x = false;
		}  else if (Math.abs(nowPage.x - this.startPage.x) - Math.abs(nowPage.y - this.startPage.y) > 0){
			this.isDir.y = false;
		}
		this.isDir.x&&setScroll("x");
		this.isDir.y&&setScroll("y");
	...
	toEnd: function(e){ 
		...
		var target = {x:0,y:0}; 
		var dis = 0;
		target.x = this.isDir.x? getTarget("x") : this.iScroll.x;
		target.y = this.isDir.y? getTarget("y") : this.iScroll.y;
		this.iScroll.x = target.x;
		this.iScroll.y = target.y;
		dis = Math.max(Math.abs(target.x-this.iScroll.x),Math.abs(target.y-this.iScroll.y));
		function getTarget(dir){
			var speed = _this.translateDis[dir] / _this.timeDis*10;
			speed = _this.timeDis == 0 ? 0 : speed; 
			speed = Math.abs(speed) > 5? speed*22 : speed*5;
		...
		this.move(type,Math.floor(dis*1.5));
	...
	setTranslate: function(){
		...
		this.scrollBar.y&&css(this.scrollBar.y,"translateY", -this.iScroll.y * this.scale.y);
		this.scrollBar.x&&css(this.scrollBar.x,"translateX", -this.iScroll.x * this.scale.x);
	},
	move: function(type,time){
		var _this  = this;
		time = time<400?400:time;
		this.Scroll.style.transition = time+"ms "+type;
		this.scrollBar.y&&(this.scrollBar.y.style.transition = time+"ms "+type);
		this.scrollBar.x&&(this.scrollBar.x.style.transition = time+"ms "+type);
		this.setTranslate();
		this.timer = setTimeout(
			function(){
				_this.overScroll&&_this.overScroll();
				if(_this.scrollBar.x){
					_this.scrollBar.x.style.transition = ".5s 1s opacity";
					_this.scrollBar.x.style.opacity = 0;
				}
				if(_this.scrollBar.y){
					_this.scrollBar.y.style.transition = ".5s 1s opacity";
					_this.scrollBar.y.style.opacity = 0;
				}
			},time
		);
	},
	reSize: function(){
			this.minTranslate = {x: this.element.clientWidth - this.Scroll.offsetWidth, y: this.element.clientHeight - this.Scroll.offsetHeight};
			this.scale = {x:this.element.clientWidth  / this.Scroll.offsetWidth,y:this.element.clientHeight / this.Scroll.offsetHeight};
			this.scrollBar.y&&(this.scrollBar.y.style.height = this.element.clientHeight * this.scale.y +"px");
			this.scrollBar.x&&(this.scrollBar.x.style.width = this.element.clientWidth * this.scale.x +"px");
			this.setTranslate();
	}
};
